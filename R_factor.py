...
PRE_folder = r"H:\SoilErosion_TGR\Datasets\R_factor\Rainfall"
PRE_folder_out = r"H:\SoilErosion_TGR\Datasets\Rainfall\PRE"
...
for name in file_list:
    ...
    # 计算R因子
    target_data["R_factor"] = 0.2686 * (1 + np.cos(np.pi * target_data["Month"] - 7 * np.pi) / 6 
                                        ) * np.power(target_data["Rain"], 1.7263)
    ...
    target_data.drop(["Year", "Month"], axis=1, inplace=True)
    R_factor = pd.pivot_table(target_data, index="ID", aggfunc=np.sum)

    # 拼接，获取站点坐标数据
    # out_data = pd.merge(R_factor, station_coord, on="ID")

    # write out R-factor data
    R_factor_out = R_folder + "\\" + name
    R_factor.to_excel(R_factor_out, sheet_name="R_factor", index=True)
